...
def main(args):
    ...
    train, val, test = load_datasets(data_root, image_size=(224, 224), batch_size=32)
    ...
    ckpt_path = os.path.join(args.checkpoint_dir, 'ffnn_best.keras')
    callbacks = [
        ...
        tf.keras.callbacks.ReduceLROnPlateau(factor=0.5, patience=5,
                                              monitor='val_loss', mode='min'),
        tf.keras.callbacks.CSVLogger(os.path.join(args.results_dir, 'ffnn_training.csv'))
    ]
    ...
    model.save(os.path.join(args.checkpoint_dir, 'ffnn_final.keras'))

    plot_training_history(history, save_path=os.path.join(args.results_dir, 'ffnn_history.png'))
    ...
    plot_confusion_matrix(y_true, y_pred, class_names,
                          save_path=os.path.join(args.results_dir, 'ffnn_cm.png'))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--data-root', default='data/split')
    parser.add_argument('--results-dir', default='results/ffnn')
